...
// Create the application state management object
// This function must be called once inside BaseLayout
export function createState(inParentComponent) {
    ...
    return {
        ...
        // Message list for the current mailbox
        messages: [],

        // Current view ("welcome", "message", "compose", "contact", "contactAdd", or "contactEdit")
        currentView: "welcome",
        ...
        showMessage: async function (inMessage: IMAP.IMessage): Promise<void> {
            ...
            // Update state with message details
            this.setState({
                currentView: "message",
        ...
        showComposeMessage: function (inType: string): void {
            ...
            switch (inType) {
                ...
                case "reply":
                    // Auto-fill recipient and subject for replies
                    this.setState({
                        currentView: "compose",
                ...
                case "contact":
                    // Auto-fill recipient when emailing a contact
                    this.setState({
                        currentView: "compose",
        ...
        addMailboxToList: function (inMailbox: IMAP.IMailbox): void {
            ...
            // Filter out the [Gmail] parent folder to avoid issues
        ...
        getMessages: async function (inPath: string): Promise<void> {
            ...
            // Format and sort messages by date (newest first)
            const formattedMessages = messages.map((inMessage: IMAP.IMessage) => ({
                id: inMessage.id,
                date: inMessage.date,
                from: inMessage.from,
        ...
        updateContact: async function (): Promise<void> {
            ...
            const cl = this.state.contacts.map((c) =>
                c._id === this.state.contactID
                    ? { _id: updatedContact._id, name: updatedContact.name, email: updatedContact.email }
                    : c
            );

            this.setState({
                contacts: cl, currentView: "contact",
                contactName: updatedContact.name, contactEmail: updatedContact.email
            });
